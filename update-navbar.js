...
// 3. Replace the nav mapping in HTML
const htmlMapping = `{navItems.map((item) => (
        item.isDropdown ? (
          ...
          >
            {item.label}
            {item.label === 'three.js' && (
              <span class="badge-nuevo" style="position: absolute; top: -5px; right: -8px; background: #FC5D7D; color: white; font-size: 10px; padding: 1px 4px; border-radius: 0; font-weight: 100; margin-left: 4px; font-family: var(--font-afronaut, system-ui, sans-serif);">Nuevo</span>
            )}
            {item.label === 'Keyframes' && (
              <span style="position: absolute; top: -5px; right: -8px; background: #9FA6FF; color: var(--color-bg); font-size: 10px; padding: 1px 4px; border-radius: 0; font-weight: 100; margin-left: 4px; transform: rotate(10deg); font-family: var(--font-afronaut, system-ui, sans-serif);">Principiante</span>
            )}
            {item.label === 'Transform' && (
              <span style="position: absolute; top: -5px; right: -8px; background: #57B0FB; color: var(--color-bg); font-size: 10px; padding: 1px 4px; border-radius: 0; font-weight: 100; margin-left: 4px; transform: rotate(10deg); font-family: var(--font-afronaut, system-ui, sans-serif);">Avanzado</span>
            )}
          </a>
        )
      ))}`;
content = content.replace(/\{navItems\.map\(\(item\) => \([\s\S]*?\)\)\}/, htmlMapping);

// Replace mobile mapping as well
const mobileHtmlMapping = `{navItems.map((item) => (
      item.isDropdown ? (
        <div class="mobile-dropdown-group" style="display:flex; flex-direction:column; gap:8px;">
...
content = content.replace(/\{navItems\.map\(\(item\) => \([\s\S]*?<\/a>\s*\)\s*\)\}/, mobileHtmlMapping);

// 4. Update the JavaScript logic
const scriptLogic = `// Update active links based on the current URL (since navbar is persisted)
    ...
    navLinks?.querySelectorAll('[data-nav-item]').forEach((link) => {
      if (link.hasAttribute('data-dropdown-btn')) {
        // This is the parent button. We'll set its active state below based on its children.
        return;
      }
...
content = content.replace(/\/\/ Update active links based on the current URL[\s\S]*?indicator\.classList\.add\('visible'\);\n    }/, scriptLogic);

// Wait, the HTML for a normal <a> inside nav-links also changed. I'll make sure there's no conflict.
